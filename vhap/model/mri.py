...
class MRIHead(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "lmk_template_coords",  # 改为与forward中一致的名称
            torch.tensor(mri_lmks, dtype=self.dtype)
        )
        ...
        # laplacian matrix
        laplacian_matrix = Meshes(verts=[self.v_template], faces=[
                                  faces.verts_idx]).laplacian_packed().to_dense()
        self.register_buffer("laplacian_matrix",
                             laplacian_matrix, persistent=False)

        D = torch.diag(laplacian_matrix)
        laplacian_matrix_negate_diag = laplacian_matrix - torch.diag(D) * 2
        self.register_buffer("laplacian_matrix_negate_diag",
                             laplacian_matrix_negate_diag, persistent=False)
    ...
    def add_teeth(self):
        ...
        vid_lip_outside_ring_upper = self.mask.get_vid_by_region(
            ['lip_outside_ring_upper'], keep_order=True)

        vid_lip_outside_ring_lower = self.mask.get_vid_by_region(
        ...
        mean_dist = (v_lip_upper - v_lip_lower).norm(dim=-
                                                     1, keepdim=True).mean()
        ...
        # how far the teeth are from the lips
        v_teeth_middle[:, 2] -= mean_dist * 1.5

        # upper, front
        v_teeth_upper_edge = v_teeth_middle.clone(
        ) + torch.tensor([[0, mean_dist, 0]])*0.1
        v_teeth_upper_root = v_teeth_upper_edge + \
            torch.tensor([[0, mean_dist, 0]]) * 2  # scale the height of teeth

        # lower, front
        v_teeth_lower_edge = v_teeth_middle.clone(
        ) - torch.tensor([[0, mean_dist, 0]])*0.1
        ...
        v_teeth_lower_root = v_teeth_lower_edge - \
            torch.tensor([[0, mean_dist, 0]]) * 2  # scale the height of teeth
        ...
        vid_teeth_upper = torch.cat([vid_teeth_upper_root, vid_teeth_upper_edge,
                                    vid_teeth_upper_root_back, vid_teeth_upper_edge_back], dim=0)
        vid_teeth_lower = torch.cat([vid_teeth_lower_root, vid_teeth_lower_edge,
                                    vid_teeth_lower_root_back, vid_teeth_lower_edge_back], dim=0)
        ...
        # v = v[[0, 2, 1, 1]]
        # v = v[[0, 3, 1, 4, 3, 2, 6, 5]]
        # TODO: with this order, teeth_lower is not rendered correctly in the uv space
        v = v[[3, 2, 0, 1, 3, 4, 6, 5]]
        uv = torch.stack(torch.meshgrid(u, v, indexing='ij'), dim=-
                         1).permute(1, 0, 2).reshape(num_verts_teeth, 2)  # (#num_teeth, 2)
        ...
        self.shapedirs = torch.cat(
            [self.shapedirs, torch.zeros_like(self.shapedirs[:num_verts_teeth])], dim=0)
        shape_dirs_mean = (self.shapedirs[vid_lip_outside_ring_upper, :, :self.n_shape_params] +
                           self.shapedirs[vid_lip_outside_ring_lower, :, :self.n_shape_params]) / 2
        self.shapedirs[vid_teeth_upper_root, :,
                       :self.n_shape_params] = shape_dirs_mean
        self.shapedirs[vid_teeth_lower_root, :,
                       :self.n_shape_params] = shape_dirs_mean
        self.shapedirs[vid_teeth_upper_edge, :,
                       :self.n_shape_params] = shape_dirs_mean
        self.shapedirs[vid_teeth_lower_edge, :,
                       :self.n_shape_params] = shape_dirs_mean
        self.shapedirs[vid_teeth_upper_root_back, :,
                       :self.n_shape_params] = shape_dirs_mean
        self.shapedirs[vid_teeth_upper_edge_back, :,
                       :self.n_shape_params] = shape_dirs_mean
        self.shapedirs[vid_teeth_lower_root_back, :,
                       :self.n_shape_params] = shape_dirs_mean
        self.shapedirs[vid_teeth_lower_edge_back, :,
                       :self.n_shape_params] = shape_dirs_mean

        # posedirs set to zero
        posedirs = self.posedirs.reshape(
            len(self.parents)-1, 9, num_verts_orig, 3)  # (J*9, V*3) -> (J, 9, V, 3)
        posedirs = torch.cat([posedirs, torch.zeros_like(
            posedirs[:, :, :num_verts_teeth])], dim=2)  # (J, 9, V+num_verts_teeth, 3)
        self.posedirs = posedirs.reshape(
            (len(self.parents)-1)*9, (num_verts_orig+num_verts_teeth)*3)  # (J*9, (V+num_verts_teeth)*3)

        # J_regressor set to zero
        self.J_regressor = torch.cat([self.J_regressor, torch.zeros_like(
            self.J_regressor[:, :num_verts_teeth])], dim=1)  # (5, J) -> (5, J+num_verts_teeth)

        # lbs_weights manually set
        self.lbs_weights = torch.cat([self.lbs_weights, torch.zeros_like(
            self.lbs_weights[:num_verts_teeth])], dim=0)  # (V, 5) -> (V+num_verts_teeth, 5)
        ...
        self.faces = torch.cat(
            [self.faces, f_teeth_upper+num_verts_orig, f_teeth_lower+num_verts_orig], dim=0)
    ...
    def forward(
        ...
        if return_landmarks:
            ...
            # 扩展地标模板到batch维度
            landmarks = self.lmk_template_coords.unsqueeze(
                0).expand(batch_size, -1, -1)

            # 应用与mesh相同的刚性变换
            if zero_centered_at_root_node:
                landmarks = landmarks - J[:, [0]]  # 与mesh同样的根节点居中

            landmarks = landmarks + translation[:, None, :]  # 与mesh同样的全局平移

            # np.save('landmarks_template.npy',
            #                 landmarks.squeeze(0).detach().cpu().numpy())
            # import os
            # import numpy as np
            # external_lmk_path = "/home/zhihao/NeRSemble/visualize_landmarks/full_landmark_points.npy"

            # # 你的npz文件路径
            # if os.path.exists(external_lmk_path):
            #     lmks_npz = np.load(external_lmk_path)
            #     # 自动判断是npz还是npy
            #     if hasattr(lmks_npz, 'files') and 'lmks' in lmks_npz.files:
            #         lmks_arr = lmks_npz['lmks']
            #     else:
            #         lmks_arr = lmks_npz  # 直接是ndarray
            #     # 扩展为 [batch, 70, 3]
            #     bz = vertices.shape[0]
            #     if lmks_arr.ndim == 2:
            #         lmks_arr = np.expand_dims(lmks_arr, axis=0)
            #     if lmks_arr.shape[0] != bz:
            #         assert lmks_arr.shape[0] == 1, "external landmarks batch size不匹配"
            #         lmks_arr = np.repeat(lmks_arr, bz, axis=0)
            #     external_landmarks = torch.tensor(
            #         lmks_arr,
            #         dtype=vertices.dtype,
            #         device=vertices.device
            #     )
            #     landmarks = external_landmarks
            # else:
            #     bz = vertices.shape[0]
            #     landmarks = vertices2landmarks(
            #         vertices,
...
class FlameTexPainted(nn.Module):
    def __init__(self, tex_size=512, painted_tex_path=FLAME_PAINTED_TEX_PATH):
        ...
        tex_painted = torch.tensor(
            np.array(Image.open(painted_tex_path))[:, :, :3]) / 255
        tex_painted = tex_painted[None, ...].permute(0, 3, 1, 2)
        if tex_painted.shape[-1] != self.tex_size or tex_painted.shape[-2] != self.tex_size:
            tex_painted = F.interpolate(
                tex_painted, [self.tex_size, self.tex_size])
...
class FlameMask(nn.Module):
    ...
    def process_vertex_mask(self, flame_parts_path):
        ...
        part_masks = np.load(
            flame_parts_path, allow_pickle=True, encoding="latin1")
    ...
    def create_custom_mask(self):
        ...
        self.v.register_buffer(
            "front_middle_bottom_point_boundary", torch.tensor([3285]))
        self.v.register_buffer(
            "back_middle_bottom_point_boundary", torch.tensor([3248]))
        ...
        uniques, counts = torch.cat(
            [self.v.scalp, face_and_neck]).unique(return_counts=True)
        intersection = uniques[counts == 2]
        uniques, counts = torch.cat(
            [self.v.scalp, intersection]).unique(return_counts=True)
        hair = uniques[counts == 1]
        self.v.register_buffer("hair", hair)

        # unions
        self.v.register_buffer("ears", torch.cat(
            [self.v.right_ear, self.v.left_ear]))
        self.v.register_buffer("eyeballs", torch.cat(
            [self.v.right_eyeball, self.v.left_eyeball]))
        self.v.register_buffer("irises", torch.cat(
            [self.v.right_iris, self.v.left_iris]))
        self.v.register_buffer("left_eye", torch.cat(
            [self.v.left_eye_region, self.v.left_eyeball]))
        self.v.register_buffer("right_eye", torch.cat(
            [self.v.right_eye_region, self.v.right_eyeball]))
        self.v.register_buffer("eyelids", torch.cat(
            [self.v.left_eyelid, self.v.right_eyelid]))
        ...
        uniques, counts = torch.cat(
            [self.v.eyeballs, self.v.irises]).unique(return_counts=True)
        intersection = uniques[counts == 2]
        uniques, counts = torch.cat(
            [self.v.eyeballs, intersection]).unique(return_counts=True)
    ...
    def process_face_mask(self, faces):
        ...
        for region_name, f_mask in face_masks.items():
            self.f.register_buffer(
                region_name, torch.tensor(f_mask, dtype=torch.long))
    ...
    def process_face_clusters(self, face_clusters):
        ...
        # faces are always treated as foreground
        fid2cid = torch.ones(self.num_faces+1, dtype=torch.long)
        for cid, cluster in enumerate(face_clusters):
            try:
                ...
                continue
            # reserve cluster #0 for the background and #1 for faces that do not belong to any cluster
            fid2cid[fids] = cid + 2
    ...
    def process_vt_mask(self, faces, faces_t):
        ...
        for region_name, vt_mask in vt_masks.items():
            self.vt.register_buffer(
                region_name, torch.tensor(vt_mask, dtype=torch.long))
    ...
    def get_vid_except_region(self, regions):
        if isinstance(regions, str):
            regions = [regions]
        if len(regions) > 0:
            indices = torch.cat([self.v.get_buffer(k)
                                for k in regions]).unique()
        else:
    ...
    def get_fid_except_region(self, regions):
        if isinstance(regions, str):
            regions = [regions]
        if len(regions) > 0:
            indices = torch.cat([self.f.get_buffer(k)
                                for k in regions]).unique()
        else:
            indices = torch.tensor([], dtype=torch.long)
...
class FlameUvMask(BufferContainer):
    def __init__(self, uv_mask_path=FLAME_UVMASK_PATH):
        ...
        for region_name, uv_mask in uv_masks.items():
            self.register_buffer(
                region_name, torch.tensor(uv_mask, dtype=torch.bool))
