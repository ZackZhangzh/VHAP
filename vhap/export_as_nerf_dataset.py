...
class NeRFDatasetWriter:
    ...
    def write(self):
        ...
        for i, item in tqdm(enumerate(self.dataloader), total=len(self.dataloader)):
            ...
            # extrinsic[:3, 3] *= 0.1348  # scale to the original size
...
class TrackedFLAMEDatasetWriter:
    def __init__(
        ...
        if "scaling" in self.flame_params:
            self.scaling = self.flame_params["scaling"].item()
        else:
            self.scaling = None
    ...
    def scaling_cam_params(self, item):
        RT = np.array(item["transform_matrix"])
