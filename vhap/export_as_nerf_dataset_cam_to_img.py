...
class NeRFDatasetWriter:
    ...
    def write(self):
        ...
        camera_indices = set()
        time_length = len(self.dataloader.dataset.timestep_ids)
        print(f"Processing {time_length} timesteps...")
        for i, timestep_id in tqdm(
            enumerate(self.dataloader.dataset.timestep_ids),
            total=len(self.dataloader.dataset.timestep_ids),
        ):
            # Get data for this timestep
            item = self.dataloader.dataset.getitem_by_timestep(i)

            # Handle both single camera and multi-camera (batch) cases
            if isinstance(item["camera_index"], torch.Tensor):
                camera_indices_list = item["camera_index"].tolist()
            else:
                camera_indices_list = [item["camera_index"]]

            if isinstance(item["timestep_index"], torch.Tensor):
                timestep_indices_list = item["timestep_index"].tolist()
            else:
                timestep_indices_list = [item["timestep_index"]]

            num_cameras = len(camera_indices_list)

            # Process each camera for this timestep
            for camera_idx in range(num_cameras):
                camera_index = camera_indices_list[camera_idx]
                timestep_index = (
                    timestep_indices_list[camera_idx]
                    if len(timestep_indices_list) == num_cameras
                    else timestep_indices_list[0]
                )

                timestep_indices.add(timestep_index)
                camera_indices.add(camera_index)

                # Get camera intrinsics and extrinsics
                if (
                    isinstance(item["intrinsic"], torch.Tensor)
                    and item["intrinsic"].ndim > 2
                ):
                    intrinsic = item["intrinsic"][camera_idx].double().numpy()
                else:
                    intrinsic = item["intrinsic"].double().numpy()

                if (
                    isinstance(item["extrinsic"], torch.Tensor)
                    and item["extrinsic"].ndim > 2
                ):
                    extrinsic = item["extrinsic"][camera_idx]
                else:
                ...
                if isinstance(item["rgb"], torch.Tensor) and item["rgb"].ndim > 3:
                    h, w = item["rgb"][camera_idx].shape[:2]
                    rgb = item["rgb"][camera_idx]
                else:
                ...
                # Get timestep ID and original index
                if isinstance(item["timestep_id"], list):
                    timestep_id_val = item["timestep_id"][camera_idx]
                else:
                    timestep_id_val = item["timestep_id"]

                if isinstance(item["timestep_index_original"], torch.Tensor):
                    timestep_index_original = (
                        item["timestep_index_original"][camera_idx].item()
                        if item["timestep_index_original"].numel() > 1
                        else item["timestep_index_original"].item()
                    )
                else:
                    timestep_index_original = item["timestep_index_original"]

                # Get camera ID
                if isinstance(item["camera_id"], list):
                    camera_id_val = item["camera_id"][camera_idx]
                else:
                    camera_id_val = item["camera_id"]

                # Create frame item for the database
                frame_item = {
                    "timestep_index": timestep_index,
                    "timestep_index_original": timestep_index_original,
                    "timestep_id": timestep_id_val,
                    ...
                    "file_path": f"images/cam_{camera_idx:04}_{timestep_index:04}.png",
                }

                path2data = {
                    str(self.tgt_folder / frame_item["file_path"]): rgb,
                }

                # Handle alpha map if present
                if "alpha_map" in item:
                    frame_item["fg_mask_path"] = (
                        f"fg_masks/cam_{camera_idx:04}_{timestep_index:04}.png"
                    )

                    if (
                        isinstance(item["alpha_map"], torch.Tensor)
                        and item["alpha_map"].ndim > 2
                    ):
                        alpha_map = item["alpha_map"][camera_idx]
                    else:
                        alpha_map = item["alpha_map"]

                    path2data[str(self.tgt_folder / frame_item["fg_mask_path"])] = (
                        alpha_map
                    )
                ...
                # --- threading
                if len(worker_args) == max_threads or (
                    i == len(self.dataloader.dataset.timestep_ids) - 1
                    and camera_idx == num_cameras - 1
                ):
...
class MaskFromFLAME:
    def __init__(
        ...
        background_color = (
            cfg_data.background_color if background_color is None else background_color
        )
    ...
    def write(self):
        ...
        for i, frame in enumerate(tqdm(self.dataloader)):
            ...
            if timestep not in t2verts:
                t2verts[timestep] = infer_flame_params(
                    self.flame_model, frame["flame_param"], [0]
                ).cuda()
            ...
            img = (
                img * mask[:, :, None]
                + self.background_tensor.cuda()[None, None, :] * (1 - mask)[:, :, None]
            )

            # Convert image to proper numpy format for saving
            img_np = img.byte().cpu().numpy()

            # overwrite the original images
            path2data = {
                str(frame["image_path"]): img_np,
            }

            if "fg_mask_path" in frame and "fg_mask" in frame:
                fg_mask = frame["fg_mask"].cuda()
                if fg_mask.shape != mask.shape:
                    mask = mask.unsqueeze(-1)  # (H, W) → (H, W, 1)
                fg_mask = fg_mask * mask

                # Convert mask to proper numpy format for saving
                fg_mask_np = fg_mask.byte().cpu().numpy()

                # overwrite the original masks
                path2data.update(
                    {
                        str(frame["fg_mask_path"]): fg_mask_np,
                    }
                )

            # Immediately write data to avoid any potential issues
            write_data(path2data)

            # Don't accumulate worker_args anymore since we write immediately
            # Only keep this for the threading section if needed
    ...
    def get_mask_tilted_line(self, verts, RT, K, h, w):
        ...
        verts_xy_left = verts_xy[
            self.flame_model.mask.get_vid_by_region(["neck_right_point"])
        ]
        verts_xy_right = verts_xy[
            self.flame_model.mask.get_vid_by_region(["neck_left_point"])
        ]
        verts_xy_bottom = verts_xy[
            self.flame_model.mask.get_vid_by_region(
                ["front_middle_bottom_point_boundary"]
            )
        ]
...
def write_data(path2data):
    for path, data in path2data.items():
        ...
        if path.suffix in [".png", ".jpg"]:
            # Ensure data is in the correct format for PIL
            ...
            # Ensure data is in the right shape for an image
            if len(data.shape) == 3 and data.shape[0] == 3:  # CHW format
...
def split_json(tgt_folder: Path, train_ratio=0.7):
    ...
    for f in db["frames"]:
        if f["timestep_index"] in range_train:
            ...
            assert (
                f["camera_index"] in db_test["camera_indices"]
...
def main(
    ...
    nerf_dataset_writer = NeRFDatasetWriter(
        cfg.data, tgt_folder, subset, scale_factor, background_color
    )
    nerf_dataset_writer.write()

    flame_dataset_writer = TrackedFLAMEDatasetWriter(
        cfg.model, src_folder, tgt_folder, mode=flame_mode, epoch=epoch
    )
    flame_dataset_writer.write()

    if create_mask_from_mesh:
        mask_generator = MaskFromFLAME(
            cfg.data, cfg.model, tgt_folder, background_color
        )
